...
<%@ page import="com.project.entities.DraftNewsSelector" %>
...
<head>
  ...
  <title>News filtered by Author</title>
...
<%
  String selectedAuthor = request.getParameter("selected_author");
  if (selectedAuthor != null) {
    selectedAuthor = URLDecoder.decode(selectedAuthor, StandardCharsets.UTF_8);
  }
  Map<String, List<News>> categorizedNewsList = obj.getCategorizedNewsByAuthor(selectedAuthor);
%>

<div class="container mt-5">
  <div class="row">
    <div class="col-lg-9">
      <% if (categorizedNewsList != null && !categorizedNewsList.isEmpty()) { %>
      <% for (Map.Entry<String, List<News>> entry : categorizedNewsList.entrySet()) { %>
      <div class="card w-100 mt-3">
        <div class="card-header" data-category="<%=entry.getKey()%>" style="text-align: center; font-size: 1.5em; font-weight: bold;">
          <%=entry.getKey()%>
        </div>
        <div class="card-body">
          <% for (News n : entry.getValue()) { %>
