...
<%@ page import="com.project.newsScraper.ContentBlock" %>
...
<%@page import="com.project.servlets.SimilarNewsServlet"%>
...
<%
    ...
    PreparedStatement newsStmt = conn.prepareStatement("SELECT * FROM news WHERE news_id = ?");
    newsStmt.setInt(1, newsId);
    ...
    multimediaStmt.setInt(1, newsId);
    ResultSet multimediaRs = multimediaStmt.executeQuery();
...
<div class="container my-4">
    <h2><%= newsRs.getString("news_title") %></h2>
    <div class="news-content" style=" font-family: 'Barlow', sans-serif;">
        <%
            contentJson = newsRs.getString("news_content");
            Type listType = new TypeToken<ArrayList<ContentBlock>>(){}.getType();
            List<ContentBlock> contentBlocks = new Gson().fromJson(contentJson, listType);

            // Display the news content
            for (ContentBlock block : contentBlocks) {
                if (block.getType().equals("text")) {
                    out.println("<p style=\"padding:0\">" + block.getContent() + "</p>");
                } else {
                    out.println("<img src=\"" + block.getContent() + "\" alt=\"News image\" height=\"450px\" width=\"700px\">");
                }
            }
            System.out.println("Current news_id: " + newsRs.getInt("news_id"));
        %>
    </div>
<div class="container my-4">
    <form action="SimilarNewsServlet" method="GET">
        <input type="hidden" name="news_id" value="<%= newsRs.getInt("news_id") %>">
        <input type="submit" value="See similar news" class="btn" style="background-color: #FF3131;
    color: #FFFFFF;
    display: block;
    height: auto;
    width: 25%;
...
<% } else { %>
<div class="container my-4">
    <p>No news found with the given ID.</p>
</div>
<% } %>
...
<% multimediaRs.close(); multimediaStmt.close(); newsRs.close(); newsStmt.close(); %>
</html>
